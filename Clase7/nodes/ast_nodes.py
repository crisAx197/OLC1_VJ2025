...
from nodes.reporte_ast import ASTGraph
...
class Numero(Expresion):
    ...
    def graficar(self, grafo):
        return grafo.agregar_nodo(f"Numero\n{self.valor}", fillcolor="lightblue")
...
class Suma(Expresion):
    ...
    def graficar(self, grafo):
        nodo_id = grafo.agregar_nodo("Suma", fillcolor="lightgreen")
        izq_id = self.izquierda.graficar(grafo)
        der_id = self.derecha.graficar(grafo)
        grafo.agregar_arista(nodo_id, izq_id)
        grafo.agregar_arista(nodo_id, der_id)
        return nodo_id
...
class While(Expresion):
    ...
    def __str__(self):
        return f"while_{self.id}: while ({self.condicion}) {{ {self.instrucciones} }}"

    def __repr__(self):
        return f"While#{self.id}(condición={self.condicion!r}, instrucciones={self.instrucciones!r})"

from nodes.reporte_ast import graficar_ast
ast = Suma(Numero(3), Suma(Numero(5), Numero(2)))
graficar_ast(ast)
